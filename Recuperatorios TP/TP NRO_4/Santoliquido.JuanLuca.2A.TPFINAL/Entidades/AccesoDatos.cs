...
namespace Entidades
{
    public class AccesoDatos
    {
        ...
        public event Delegado SinTelaEvent;
        private Fabrica FB = new Fabrica();
        public double metrosTela = 0;
           ...
           try
           {
               ...
               this.comando.Connection = this.conexion;
                ...
                if (c is Joggings)
                {
                    ...
                    metrosTela = c.S * this.FB.costo.JT1 + c.M * this.FB.costo.JT2 + c.L * this.FB.costo.JT3 + c.XL * this.FB.costo.JT4 + c.XXL * this.FB.costo.JT5;

                }else
                {
                    metrosTela = c.S * this.FB.costo.BT1 + c.M * this.FB.costo.BT2 + c.L * this.FB.costo.BT3 + c.XL * this.FB.costo.BT4 + c.XXL * this.FB.costo.BT5;
                }
                if(this.SinTelaEvent!=null)
                {
                    this.SinTelaEvent(metrosTela, EventArgs.Empty);
            ...
            catch (TelaInsuficienteException ex)
            {
                save = false;
                throw new TelaInsuficienteException(ex.ToString());
            }
            catch (Exception ex)
           {
               save = false;
